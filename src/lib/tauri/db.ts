...
export const dbRepo = {
  ...
  // -------------------------------------------------------------------------
  // One-time migration from legacy tauriStore (plugin-store JSON) to SQLite
  // -------------------------------------------------------------------------

  /**
   * Idempotent. Reads the legacy `settings.json` store and inserts all data
   * into SQLite. Sets `app_meta.store_migrated = '1'` on success so subsequent
   * calls are no-ops. The orchestrator is responsible for calling this at
   * bootstrap time.
   */
  async migrateFromStore(): Promise<void> {
    const already = await dbRepo.getMeta('store_migrated')
    if (already === '1') return

    // Dynamic import keeps this module's top-level free of tauriStore side-effects
    const { tauriStore } = await import('./store')

    const [scanRoots, launchers, projects] = await Promise.all([
      tauriStore.get('scanRoots'),
      tauriStore.get('launchers'),
      tauriStore.get('projects'),
    ])
    ...
    try {
      ...
      // 3. Launchers (no FK deps; sort_order = array index from store)
      for (let i = 0; i < launchers.length; i++) {
        ...
        await db.execute(
          `INSERT INTO launchers (id, name, icon, command_template, sort_order, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           ON CONFLICT(id) DO NOTHING`,
          [l.id, l.name, l.icon, l.commandTemplate, i, now, now]
        )
      }
    ...
    await dbRepo.setMeta('store_migrated', '1')
  },
}
